<template>
  <div class="toolbar">
    <a-row type="flex">
      <a-col :span="1"></a-col>
      <a-col :span="11">
        <a-button type="link" @click="saveTopo" :disabled="AllowSaving || readOnly" size="small"><a-icon type="save" style="font-size: larger"/><span style="font-size: 14px">保存</span></a-button>&nbsp;&nbsp;&nbsp;&nbsp;
        <a-button type="link" @click="runTask"  v-show="!AllowRunning" :disabled="AllowSaving || readOnly" size="small"> <a-icon type="play-circle"/><span style="font-size: 14px">运行</span></a-button>&nbsp;&nbsp;&nbsp;&nbsp;
        <a-button type="link" @click="stopTask" :disabled="AllowStop || readOnly" v-show="AllowRunning" size="small"> <a-icon type="stop"/><span style="font-size: 14px">停止</span></a-button>&nbsp;&nbsp;
        <a-button type="link" @click="showNodeLog" :disabled="readOnly" size="small"> <a-icon type="code"/><span style="font-size: 14px">日志</span></a-button>
      </a-col>
      <a-col :span="11" style="text-align: right;font-size: larger">
        ...
        <i
          ...
          :class="readOnly || multiSelect?'disable':''"
          title="多选"
    ...
    <a-modal title="运行日志"
             :visible="showLog"
             @cancel="handleCancelLog" width="700px" cancelText="关闭">
...
<script>
  ...
  import { getBox, uniqueId } from '@/components/G6Editor/utils'
  import Log from '@/components/G6Editor/components/Log'
  ...
  export default {
    ...
    methods: {
      ...
      setLog(map){;
        if(this.filterLog(map.newId)){
          this.LogList.push(map)
        }
      },
      filterLog(id){
        var LogList =  this.LogList.filter(function(pageList) {
          return Object.keys(pageList).some(function(key) {
            return String(pageList[key]).indexOf(id) > -1
          })
        })
        if(LogList.length>0){
          return false
      ...
      /*运行任务*/
      runTask() {
        this.$emit('saveTopo',{type:"runTask"});
      },
      /*保存流程信息*/
      saveTopo() {
        //启动任务中不允许重复运行和保存
        ...
        //调用父页面重新加载任务方法
        this.$emit('saveTopo', {type:"saveTopo"});
      },
      init() {
