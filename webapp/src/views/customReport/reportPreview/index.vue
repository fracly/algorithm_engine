...
<template>
  <a-spin :spinning="loading">
    <!--报表查询区域-->
    <sear-form :reportParam="reportParam" :exportExcel="reportInfo.reportType=='2'?exportExcelNoPagination:exportExcel"
               :submitQuery="reportInfo.reportType=='2'?changePage:submitQuery" ref="searchForm"></sear-form>
    <!--报表展示区域  高度暂时写死 后续修改为自适应-->
    <div class="cutControl" style="height: 900px">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 加载报表信息
      loadReport() {
        loadReport({'status': this.status, reportCode: this.reportCode}).then(res => {
          const data = res
          if (data.code !== 1) {
            alert(data.msg)
            this.loading = false
