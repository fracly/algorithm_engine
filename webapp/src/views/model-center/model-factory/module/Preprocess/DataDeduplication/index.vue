<template>
  <a-form-model layout="vertical" class="ant-advanced-search-form" :model="params">
    <a-form-item label="列选择:">
      ...
      <a-button v-if="params.columnListAgo.length!=0" style="width: 100%;" @click="showChooseFiled">已选择<span class="filedChoose">{{ params.duplicateColumns.length }}</span>个字段</a-button>
      <!--字段选择模态框-->
      <a-modal :visible="FiledChoose" title="字段选择" @cancel="showChooseFiled" width="850px">
        ...
        <field-selection v-if="FiledChoose" :chooseField="params.duplicateColumns" :FiledList="params.columnListAgo" ref="filedSelect"></field-selection>
      </a-modal>
    </a-form-item>
  </a-form-model>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveChooseFiled () {
        this.params.columnList = this.params.columnListAgo
        this.params.duplicateColumns = this.$refs.filedSelect.chooseFileds
        this.FiledChoose = !this.FiledChoose
      }
    }
  }
