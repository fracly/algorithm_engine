<template>
  <a-form-model layout="vertical" class="ant-advanced-search-form" :model="params">
    <a-form-item label="列选择:">
      ...
      <a-button v-if="params.columnListAgo.length!=0" style="width: 100%;" @click="showChooseFiled">已选择<span class="filedChoose">{{ params.basicColumns.length }}</span>个字段</a-button>
      <!--字段选择模态框-->
      <a-modal :visible="FiledChoose" title="字段选择" @cancel="showChooseFiled" width="850px">
        ...
        <field-selection v-if="FiledChoose" :chooseField="params.basicColumns" :FiledList="params.columnListAgo" ref="filedSelect"></field-selection>
      </a-modal>
    </a-form-item>
  </a-form-model>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveChooseFiled () {
        this.params.basicColumns = this.$refs.filedSelect.chooseFileds
        this.FiledChoose = !this.FiledChoose
        // 基本信息表的列
        const columnList = [
          { name: 'colname', type: 'string', comment: '列名' },
          { name: 'datatype', type: 'string', comment: '数据类型' },
          { name: 'totalcount', type: 'string', comment: '总个数' },
          { name: 'missingcount', type: 'string', comment: '空值数' },
          { name: 'min', type: 'string', comment: '最小值' },
          { name: 'max', type: 'string', comment: '最大值' },
          { name: 'mean', type: 'string', comment: '平均值' },
          { name: 'stddev', type: 'string', comment: '方差' },
          { name: 'sum', type: 'string', comment: '总和' }
