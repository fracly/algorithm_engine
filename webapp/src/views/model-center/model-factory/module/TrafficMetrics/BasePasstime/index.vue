<template>
  <a-form-model layout="vertical" class="ant-advanced-search-form" :model="params">
    ...
    <a-form-item label="最大限速:">
      <a-select style="width: 100%" v-model="params.dev[3]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList1" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item label="起点路口ID:">
      <a-select style="width: 100%" v-model="params.dev[4]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList1" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item label="终点路口ID:">
      <a-select style="width: 100%" v-model="params.dev[5]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList1" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-row>
  ...
  export default {
    data () {
      return {
        ...
        wrapperCol: { span: 15 },
        timeRange: [],
        signList: [{ name: '自由流通行时长', value: '1' }, { name: '稳定态通行时长', value: '2' }],
        params: {
          columnList: [],
          columnList0: [],
          columnList1: [],
    ...
    methods: {
      ...
      signChange (e) {
        const columnList = [
          { name: 'hpzl', type: 'string', comment: '号牌种类' },
          { name: 'rid', type: 'string', comment: '所属路网路段' },
          { name: 'start_cross_id', type: 'string ', comment: '开始路口' },
          { name: 'end_cross_id', type: 'string', comment: '结束路口' },
          { name: 'duration', type: 'double', comment: '通行时间' }
