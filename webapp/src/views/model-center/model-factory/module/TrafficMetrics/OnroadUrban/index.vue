<template>
  <a-form-model layout="vertical" class="ant-advanced-search-form" :model="params">
    ...
    <a-form-item label="时间类型">
      <a-radio-group style="width: 100%" v-model="params.timeParam" @change="signChange">
  ...
  export default {
    ...
    methods: {
      ...
      initPage () {
        ...
        const columnList = [
          { name: 'hphm', type: 'string', comment: '号牌号码' },
          { name: 'hpzl', type: 'string', comment: '号牌种类' },
          { name: 'sbbh', type: 'string', comment: '设备编号' },
          { name: 'jgsj', type: 'string ', comment: '过车时间' },
          { name: 'loadnet_rid', type: 'string', comment: '所属路段ID' },
          { name: 'loadnet_inter_id', type: 'string', comment: '所属路口ID' },
