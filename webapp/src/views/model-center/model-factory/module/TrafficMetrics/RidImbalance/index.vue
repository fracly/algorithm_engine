  ...
  <a-form-model layout="vertical" class="ant-advanced-search-form" :model="params">
    ...
    <a-form-item label="设备编号:">
      <a-select style="width: 100%" v-model="params.dev[0]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList2" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item label="所属路段ID:">
      <a-select style="width: 100%" v-model="params.dev[1]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList2" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item label="所属路口ID:">
      <a-select style="width: 100%" v-model="params.dev[2]" show-search optionFilterProp="children">
        <a-select-option v-for="col in params.columnList2" :value="col.name"> {{ col.name }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-row>
  ...
  export default {
    ...
    methods: {
      ...
      initPage () {
        ...
        if ('point2datasource' in datasources) {
          this.$set(this.params, 'datasource3', datasources.point2datasource)
        } else {
          this.$set(this.params, 'datasource3', 0)
        }
        ...
        if ('point1table' in tables) {
          this.params.middleTable = tables.point1table
        } else {
          this.params.middleTable = ''
        }
        if ('point2table' in tables) {
          this.params.rightTable = tables.point2table
        } else {
        ...
        // ####################组件按需加载####################
        const columnList = [
          { name: 'cross_id', type: 'string', comment: '所属路网路口' },
          { name: 'alpha', type: 'double', comment: '进口道失衡' },
          { name: 'beta', type: 'double', comment: '出口道失衡' },
          { name: 'shzs', type: 'double', comment: '失衡指数' },
          { name: 'updatetime', type: 'string', comment: '更新时间' }
        ]
      ...
      getProNodeColumList () {
        ...
        if ('point2List' in data) {
          this.$set(this.params, 'columnList2', data.point2List)
        } else {
          this.$set(this.params, 'columnList2', [])
        }
      },
