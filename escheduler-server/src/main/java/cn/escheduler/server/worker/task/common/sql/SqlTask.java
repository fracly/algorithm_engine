 ...
 */
package cn.escheduler.server.worker.task.common.sql;
...
import cn.escheduler.common.task.common.sql.SqlBinds;
import cn.escheduler.common.task.common.sql.SqlParameters;
import cn.escheduler.common.task.common.sql.SqlType;
...
public class SqlTask extends AbstractTask {
    ...
    public void handle() throws Exception {
        ...
        sqlParameters.setLocalParams(propTime);

        logger.info("localParams ---------------: {}",sqlParameters.getLocalParams());
        logger.info("localParams --------------- size : {}",sqlParameters.getLocalParams().size());
//        logger.info("localParams --------------- n : {}",localParams.get(0).getN());
//        logger.info("localParams --------------- time : {}",localParams.get(0).getTime());
    ...
    public boolean checkParams() {
        ...
        try {
            this.sqlParameters = JSONObject.parseObject(taskParams, SqlParameters.class);
        }catch (Exception e){
            this.errorMsg="组件所有参数为空，任务流程参数都为空，请初始化组件参数";
            return false;
        }
        if (sqlParameters.getSql() == null || StringUtils.isEmpty(sqlParameters.getSql())){
            logger.error("Sql is null or isEmpty");
            errorMsg="参数有误：Sql is null or isEmpty";
            return false;
        }
        return true;
    }
