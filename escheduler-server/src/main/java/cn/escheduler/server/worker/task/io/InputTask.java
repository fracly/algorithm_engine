...
public class InputTask extends AbstractTask {
    ...
    public InputTask(TaskProps props, Logger logger) {
        super(props, logger);
        if(!checkParams()){
            exitStatusCode = -2;
            return;
        ...
        this.inputParameters = JSONObject.parseObject(props.getTaskParams(), InputParameters.class);
        if (!inputParameters.checkParameters()) {
    ...
    public void handle(){
        ...
        logger.info(inputParameters.toString());
        // determine whether there is a data source
        if (inputParameters.getDatasource() == 0 || StringUtils.isEmpty(inputParameters.getTable())){
            logger.error("datasource is null or not specify a table");
            errorMsg="参数有误：datasource is null or not specify a table";
            exitStatusCode = -2;
        }else {
            DataSource dataSource = processDao.findDataSourceById(inputParameters.getDatasource());
            Map<String, String> map = JSONUtils.toMap(dataSource.getConnectionParams());
            String table = inputParameters.getTable();
            if(!table.contains(".")){
               inputParameters.setTable(map.get("database") + "." + table);
            }
    ...
    public boolean checkParams() {
        ...
        try {
            this.inputParameters = JSONObject.parseObject(taskParams, InputParameters.class);
        }catch (Exception e){
