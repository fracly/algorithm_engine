...
public class TaskScheduleThread implements Callable<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        try {
            ...
            if(task.getExitStatusCode() != Constants.EXIT_CODE_PARMAS_ERROR){
                // job init
                task.init();
                // job handle
                task.handle();
            ...
            //组件参数错误 无法执行
            else if(task.getExitStatusCode() == Constants.EXIT_CODE_PARMAS_ERROR){
                status = ExecutionStatus.PARAMS_ERROR;
                //重新变成系统可以识别的值
                task.setExitStatusCode(-1);
            }else {
                status = ExecutionStatus.FAILURE;
            }
        }catch (Exception e){
            task.setErrorMsg("task escheduler failure :" + e.getMessage());
        ...
        AbstractParameters parameters = task.getParameters();

        //参数错误 更新状态
        String result="";
        if(ExecutionStatus.PARAMS_ERROR.equals(status) || ExecutionStatus.FAILURE.equals(status)){
            result = task.errorMsg;
        }else{
            result =parameters.getResultTable();
        }
        //
        //logger.info("biaozhi:获取结果表为" + parameters.getResultTable());
        processDao.changeTaskState(status,
                new Date(), taskInstance.getId(),result);
