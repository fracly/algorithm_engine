...
public class ProjectUserMapperProvider {
  ...
  private static final String TABLE_DEFINITION = "t_escheduler_process_definition";
   ...
   * @return
   */
  public String queryDefinition(Map<String, Object> parameter) {
    return new SQL() {{
      SELECT(" p.id,p.name,p.`desc`,u.name as projectName ");
      FROM(TABLE_DEFINITION +" p");
      JOIN(PROJECT_TABLE_NAME + " u on p.project_id = u.id");
      int projectId = (int) parameter.get("projectId");
      if(projectId != 0){
        WHERE("`project_id` = #{projectId} ");
      }
     ...
     * @return
     */
    public String queryFlagDefinition(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT(" p.id,p.name,p.`desc`,u.name as projectName,n.id as userId,n.user_name as userName ");
            FROM(TABLE_DEFINITION +" p");
            JOIN(PROJECT_TABLE_NAME + " u on p.project_id = u.id");
            JOIN(USER_TABLE_NAME + " n on p.user_id = n.id");
            WHERE("p.`flag` = 0 ");
        }}.toString();
    }
}
