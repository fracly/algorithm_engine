...
public class TaskInstanceMapperProvider {
    ...
    private static final String TABLE_PROJECT = "t_escheduler_project";
    /**
     ...
     * @return
     */
    public String getAppIds(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("app_link as appId");
                FROM(TABLE_NAME);
     ...
     * @return
     */
    public String findValidTaskListByProcessId2(Map<String, Object> parameter) {
        String taskNames = parameter.get("taskName").toString();
        return new SQL()
        {
            {
                SELECT("*, UNIX_TIMESTAMP(end_time)-UNIX_TIMESTAMP(start_time) as duration");
                FROM(TABLE_NAME );
                WHERE(" id in(select max(id) id from(select b.* from\n" +
                        " (select name from t_escheduler_task_instance where process_definition_id=(select process_definition_id " +
                        "from t_escheduler_process_instance where id=#{processInstanceId}) and name not in("+taskNames+") group by name ) a \n" +
                        " LEFT JOIN (select * from t_escheduler_task_instance where process_definition_id=(select process_definition_id from t_escheduler_process_instance where id=#{processInstanceId}) ORDER BY id desc) b \n" +
                        " on a.name =b.name) t group by name,task_type) ");
                WHERE("`flag` = " + EnumFieldUtil.genFieldStr("flag", Flag.class));
                ORDER_BY("start_time desc");
            }
        }.toString();
     ...
     * @return
     */
    public String findValidTaskListByProcessIdIn(Map<String, Object> parameter) {
        String taskNames = parameter.get("taskName").toString();
        return new SQL()
        {
            {
                SELECT("*, UNIX_TIMESTAMP(end_time)-UNIX_TIMESTAMP(start_time) as duration");
                FROM(TABLE_NAME );
                WHERE("process_instance_id=#{processInstanceId} and name in("+taskNames+")  ");
                WHERE("`flag` = " + EnumFieldUtil.genFieldStr("flag", Flag.class));
                ORDER_BY("start_time desc");
            }
        }.toString();
     ...
     * @return
     */
    public String queryTaskInstanceListPaging2(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("instance.*,define.name as process_instance_name, UNIX_TIMESTAMP(instance.end_time)-UNIX_TIMESTAMP(instance.start_time) as duration");

                FROM("t_escheduler_process_definition define JOIN\n" +
                        "(select tab.id from (\n" +
                        "SELECT  p.id \n" +
                        "from t_escheduler_project p\n" +
                        "where p.user_id ="+ parameter.get("userId")+" union SELECT  pu.project_id\n" +
                        "from t_escheduler_relation_project_user pu\n" +
                        "where user_id = "+ parameter.get("userId")+" )tab ) t \n" +
                        "join t_escheduler_task_instance instance\n" +
                        "on t.id=define.project_id and instance.process_definition_id=define.id");
...
//                JOIN(INSTANCE_TABLE_NAME + " process on process.id=instance.process_instance_id");
                ...
                if(parameter.get("projectId") != null && (int)parameter.get("projectId") != 0){
                    JOIN(TABLE_PROJECT + " p ON p.id = define.project_id");
                    WHERE( "p.id = #{projectId} ");
                }
     ...
     * @return
     */
    public String countTaskInstance2(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("count(1)");
                FROM("t_escheduler_process_definition define JOIN\n" +
...
//                JOIN(DEFINE_TABLE_NAME + " define ON instance.process_definition_id = define.id");
     ...
     * @return
     */
    public String queryByInstanceIdAndName2(Map<String, Object> parameter){
        return new SQL(){
            {
                SELECT("*,UNIX_TIMESTAMP(end_time)-UNIX_TIMESTAMP(start_time) as duration");
                FROM(TABLE_NAME);
    ...
    public String queryByProcessInstance(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("instance.*,define.name as process_instance_name, UNIX_TIMESTAMP(instance.end_time)-UNIX_TIMESTAMP(instance.start_time) as duration");
            FROM("t_escheduler_process_definition define JOIN" +
                    "(select tab.id from (" +
                    "SELECT  p.id " +
                    "from t_escheduler_project p union SELECT  pu.project_id " +
                    "from t_escheduler_relation_project_user pu)tab ) t " +
                    "join t_escheduler_task_instance instance " +
                    "on t.id=define.project_id and instance.process_definition_id=define.id");
            WHERE("instance.process_instance_id=#{processInstanceId}");
        }}.toString();
