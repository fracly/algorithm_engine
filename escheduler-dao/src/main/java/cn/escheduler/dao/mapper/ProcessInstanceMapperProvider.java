...
public class ProcessInstanceMapperProvider {
     ...
     * @return
     */
    public String queryProcessInstanceListPaging2(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("instance.*,  x.name as projectName, (UNIX_TIMESTAMP(instance.end_time) - UNIX_TIMESTAMP(instance.start_time)) as duration, define.name as process_definition_name");
                ...
                FROM("t_escheduler_process_definition define JOIN\n" +
                        ...
                        "join t_escheduler_process_instance instance\n" +
                        "on t.id=define.project_id and instance.process_definition_id=define.id\n" +
                        "join t_escheduler_project x on define.project_id = x.id");
...
//                JOIN(DEFINE_TABLE_NAME + " define ON instance.process_definition_id = define.id");
    ...
    public String countProcessInstance2(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("count(1)");
                FROM("t_escheduler_process_definition define JOIN\n" +
...
//                JOIN(DEFINE_TABLE_NAME + " define ON instance.process_definition_id = define.id");
    ...
    public String countProcessAvgRunTimesStatistic(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("count(distinct process_definition_id)");
            FROM(TABLE_NAME);
            WHERE("start_time >= #{startTime} and start_time <= #{endTime}");
        }}.toString();
    }

    public String countProcessTodayRunTimesStatistic(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("count(1)");
            FROM(TABLE_NAME + " a");
            INNER_JOIN(DEFINE_TABLE_NAME + " b on a.process_definition_id = b.id");
            WHERE("a.start_time >= #{startTime} and a.start_time <= #{endTime} and b.user_id = #{userId}");
        }}.toString();
    }

    public String countProcessTodaySuccessTimesStatistic(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("count(1)");
            FROM(TABLE_NAME + " a");
            INNER_JOIN(DEFINE_TABLE_NAME + " b on a.process_definition_id = b.id");
            WHERE("a.start_time >= #{startTime} and a.start_time <= #{endTime} and b.user_id = #{userId} and state = 7");
        }}.toString();
    }

    public String countProcessTodayFailedTimesStatistic(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("ifnull(count(1), 0)");
            FROM(TABLE_NAME + " a");
            INNER_JOIN(DEFINE_TABLE_NAME + " b on a.process_definition_id = b.id");
            WHERE("a.start_time >= #{startTime} and a.start_time <= #{endTime} and b.user_id = #{userId} and state = 6");
        }}.toString();
    }
}
