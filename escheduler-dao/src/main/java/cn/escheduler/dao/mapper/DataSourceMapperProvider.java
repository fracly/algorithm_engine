...
public class DataSourceMapperProvider {
  ...
  public String countUserDatasourceBySearch(Map<String, Object> parameter) {
    return new SQL() {{
      SELECT("count(0)");

      FROM(TABLE_NAME);
      WHERE("id in (select datasource_id from "+USER_DATASOURCE_RELATION_TABLE_NAME+" where user_id=#{userId} union select id as datasource_id  from "+TABLE_NAME+" where user_id=#{userId}) and name like concat('%', #{searchVal}, '%') ");
    }}.toString();
  }
  ...
  public String countAllDatasourceBySearch(Map<String, Object> parameter) {
    return new SQL() {{
      SELECT("count(0)");
      FROM(TABLE_NAME);
      WHERE(" name  like concat('%', #{searchVal}, '%')  ");
