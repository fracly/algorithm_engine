...
//            WHERE("id in (select udf_id from "+USER_UDFS_RELATION_TABLE_NAME+" where user_id=#{userId} union select id as udf_id  from "+TABLE_NAME+" where user_id=#{userId})");
            WHERE("id in (select udf_id from " + USER_UDFS_RELATION_TABLE_NAME +
                    " where user_id=#{userId} union select id as resources_id  from " + TABLE_NAME +
                    " where user_id in (SELECT id FROM "+USER_TABLE_NAME +" WHERE tenant_id IN ( SELECT tenant_id FROM "+USER_TABLE_NAME+" WHERE id = #{userId})))");
            Object searchVal = parameter.get("searchVal");
            if(searchVal != null && StringUtils.isNotEmpty(searchVal.toString())){
                WHERE( " func_name like concat('%', #{searchVal}, '%') ");
            }
