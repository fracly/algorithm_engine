...
public class ProcessDefinitionController extends BaseController{
    ...
    public Result createProcessDefinition(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
        ...
        try {
            String name = String.valueOf(jsonParam.get("name"));
            String json = JSON.toJSONString(jsonParam.get("processDefinitionJson"));
            String locations =  JSON.toJSONString(jsonParam.get("locations"));
            String desc = String.valueOf(jsonParam.get("desc"));
            String connects =  JSON.toJSONString(jsonParam.get("connects"));
     ...
     * @return
     */
    @PostMapping(value = "/saveKylin")
    @ResponseStatus(HttpStatus.CREATED)
    public Result createProcessDefinitionKylin(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                               @PathVariable String projectName,
                                               @RequestParam String cubeName) {

        try {

            Map<String, Object> result = processDefinitionService.createProcessDefinitionKylin(loginUser, projectName, cubeName);
            return returnDataList(result);
        }catch (Exception e){
            logger.error(CREATE_PROCESS_DEFINITION.getMsg(),e);
            return error(CREATE_PROCESS_DEFINITION.getCode(), CREATE_PROCESS_DEFINITION.getMsg());
    ...
    @ResponseStatus(HttpStatus.OK)
    public Result editProccessDefinition(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                           @PathVariable String projectName,
                                           @RequestBody Map<String,Object> jsonParam) {
        ...
        try {
            ...
            int id =  Integer.parseInt(String.valueOf(jsonParam.get("id")));
            Map<String, Object> result = processDefinitionService.editProcessDefinition(loginUser, projectName, id, name,desc);
            return returnDataList(result);
        }catch (Exception e){
            logger.error(UPDATE_PROCESS_DEFINITION_ERROR.getMsg(),e);
            return error(UPDATE_PROCESS_DEFINITION_ERROR.getCode(), Status.UPDATE_PROCESS_DEFINITION_ERROR.getMsg());
    ...
    @ResponseStatus(HttpStatus.OK)
    public Result shareProccessDefinition(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                            @PathVariable String projectName,
                                            @RequestParam(value = "processId", required = true) int processId,
                                            @RequestParam(value = "flag", required = true) int flag) {

        try {
            logger.info("login user {}, release process definition, project name: {}, flag: {}",
                    loginUser.getUserName(), projectName, flag);
            Map<String, Object> result = processDefinitionService.shareProcessDefinition(loginUser, projectName, processId, flag);
            return returnDataList(result);
        }catch (Exception e){
            logger.error(RELEASE_PROCESS_DEFINITION_ERROR.getMsg(),e);
            return error(RELEASE_PROCESS_DEFINITION_ERROR.getCode(), Status.RELEASE_PROCESS_DEFINITION_ERROR.getMsg());
