...
public class LoggerController extends BaseController {
    ...
    /**
     * query task execute result
     */
    @GetMapping(value = "/result")
    @ResponseStatus(HttpStatus.OK)
    public Result queryExecuteResult(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                           @RequestParam(value = "taskInstId") int taskInstanceId,
                           @RequestParam(value = "skipLineNum") int skipNum,
                           @RequestParam(value = "limit") int limit) {
        try {
            logger.info(
                    "login user {}, view {} task instance result ,skipLineNum {} , limit {}", loginUser.getUserName(), taskInstanceId, skipNum, limit);
            return loggerService.queryResult(taskInstanceId, skipNum, limit);
        } catch (Exception e) {
            logger.error(QUERY_TASK_INSTANCE_RESULT_ERROR.getMsg(), e);
            return error(QUERY_TASK_INSTANCE_RESULT_ERROR.getCode(), QUERY_TASK_INSTANCE_RESULT_ERROR.getMsg());
        }
    }
