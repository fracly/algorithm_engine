...
public class DataSourceController extends BaseController {
    ...
    public Result queryDataSourceListPaging(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
        ...
        try {
            ...
            if(("").equals(searchVal)){
                ...
                return returnDataListPaging2(result);
            }
    ...
    public Result connectDataSource(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
        ...
        try {
            ...
            if(type==DbType.ELASTICSEARCH){
                isConnection = dataSourceService.checkEsConnection(host,port);
            }else{
                String parameter = dataSourceService.buildParameter(name, note, type, host, port, database, userName, password, other);
                isConnection = dataSourceService.checkConnection(type, parameter);
            }
     ...
     * @return
     */
    @PostMapping(value = "/table-list-by-id")
    @ResponseStatus(HttpStatus.OK)
    public Result getDatabaseTableListById(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                           @RequestParam("type") DbType dbType, @RequestParam("datasourceId") int datasourceId,
                                            @RequestParam(value = "searchVal", required = false) String searchVal) {
        logger.info("login user {}, get datasource table list failure, datasourceId: {}",
                loginUser.getUserName(), datasourceId);
        DataSource dataSource = dataSourceService.queryDataSourceObject(datasourceId);
        try {
            List<Map<String, String>> tableList = dataSourceService.getTableListById(dbType, dataSource);

            List<Map<String, String>> list=new ArrayList<>();
            Result result = new Result();
            if(searchVal!=null && StringUtils.isNotEmpty(searchVal)){
                for(int i=0;i<tableList.size();i++){
                    Map map=new HashMap();
                    if (tableList.get(i).get("name").contains(searchVal)){
                        map.put("name",tableList.get(i).get("name"));
                        list.add(map);
                    }
                }
                result.setData(list);
            ...
            return error(GET_DATASOURCE_TABLE_LIST_FAILURE.getCode(), GET_DATASOURCE_TABLE_LIST_FAILURE.getMsg());
        }
    }
     ...
     * @return
     */
    @PostMapping(value = "/column-list-by-id")
    @ResponseStatus(HttpStatus.OK)
    public Result getColumnListById(@RequestAttribute(value = Constants.SESSION_USER) User loginUser, @RequestParam("type") DbType dbType,
                                    @RequestParam("datasourceId") int datasourceId, @RequestParam("table") String table) {
        logger.info("login user {}, get datasource table list failure, datasourceId: {}",
                loginUser.getUserName(), datasourceId);
        DataSource dataSource = dataSourceService.queryDataSourceObject(datasourceId);
        try {
            List<Map<String, String>> columnList = dataSourceService.getColumnListById(dbType, dataSource, table);
            Result result = new Result();
     ...
     * @return
     */
    @GetMapping(value = "/getTmp")
    @ResponseStatus(HttpStatus.OK)
    public String getTmp() {

        return ConfigurationManager.getProperty(Constants.TMP_ID);
    }
