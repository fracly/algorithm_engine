...
public class QueueController extends BaseController{
    ...
    @ResponseStatus(HttpStatus.OK)
    public Result deleteQueue(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                              @RequestParam(value = "id") int id) {
        logger.info("login user {}, update queue, id: {}, queue: {}, queueName: {}",
                loginUser.getUserName(), id);
        Result result = new Result();
        int count = queueService.deleteQueue(id);
        if(count > 0) {
            putMsg(result, CUSTOM_SUCESSS, "删除队列成功");
        } else {
            putMsg(result, CUSTOM_FAILED, "删除队列失败");
        }
        return result;
    }
