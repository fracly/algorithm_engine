...
public class ProcessInstanceController extends BaseController{
    ...
    public Result queryTaskListByProcessId(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                           @PathVariable String projectName,
                                           @RequestParam(value ="processInstanceId", required = false, defaultValue = "0") Integer workflowId,
                                           @RequestParam(value = "processDefinitionId", required = false, defaultValue = "0") Integer processDefinitionId
    ) {
//        try{
            logger.info("query task instance list by process instance id, login user:{}, project name:{}, work instance id:{}, processDefinitionId id:{}",
                    loginUser.getUserName(), projectName, workflowId,processDefinitionId);
            if(workflowId != 0){
                Map<String, Object> result = processInstanceService.queryTaskListByProcessId(loginUser, projectName, workflowId);
                return returnDataList(result);
            }else {
                Map<String, Object> result = processInstanceService.queryTaskListByProcessDefinitionId(loginUser, projectName, processDefinitionId);
                return returnDataList(result);
            }
...
//        }
    ...
    @ResponseStatus(HttpStatus.OK)
    public Result queryProcessInstatnceNodeResult(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                                            @PathVariable String projectName,
                                                            @RequestParam("processInstanceId") Integer processInstanceId,
                                                            @RequestParam("name") String name,
                                                            @RequestParam("limit") int limit){
        Result result = processInstanceService.queryProcessInstanceNodeResult(loginUser, projectName,processInstanceId, name, limit);
        return result;
    }
