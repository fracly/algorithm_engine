...
public class ProjectController extends BaseController {
    ...
    public Result createProject(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
        ...
        try {
            ...
            if(type == null) { type = 0; }
            Map<String, Object> result = projectService.createProject(loginUser, projectName, desc, type);
    ...
    @ResponseStatus(HttpStatus.OK)
    public Result queryProjectList(@RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                         @RequestParam(value = "searchVal", required = false) String searchVal
    ) {
            Result result = new Result();
            logger.info("login user {}, query project list", loginUser.getUserName());
            searchVal = ParameterUtils.handleEscapes(searchVal);
            List<Project> list = projectService.queryProjectList(loginUser, searchVal);
            result.setData(list);
     ...
     */
    @GetMapping(value = "/scheduled-project")
    @ResponseStatus(HttpStatus.OK)
    public Result queryScheduledProject(@RequestAttribute(value = Constants.SESSION_USER) User loginUser, @RequestParam("searchVal") String searchVal, @RequestParam("pageNo") Integer pageNo, @RequestParam("pageSize") Integer pageSize) {
        Result result = new Result();
        List<Schedule> scheduleList = projectService.queryProjectSchedules(loginUser.getId(), searchVal, pageNo, pageSize);
        Integer total = projectService.countProjectSchedules(loginUser.getId(), searchVal);
