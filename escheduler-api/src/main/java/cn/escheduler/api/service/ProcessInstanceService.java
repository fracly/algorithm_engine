...
import static cn.escheduler.common.enums.ExecutionStatus.FAILURE;
...
public class ProcessInstanceService extends BaseDAGService {
    ...
    public Map<String, Object> queryProcessInstanceList2(User loginUser,Integer projectId, Integer processDefineId,
                                                        String startDate, String endDate,
                                                        String searchVal, ExecutionStatus stateType, String host,
                                                        Integer pageNo, Integer pageSize) {
        ...
        Integer count = processInstanceMapper.countProcessInstance2(userId,projectId, processDefineId, statesStr,
                host, start, end, searchVal);

        PageInfo pageInfo = new PageInfo<ProcessInstance>(pageNo, pageSize);
        List<ProcessInstance> processInstanceList = processInstanceMapper.queryProcessInstanceListPaging2(
                userId,projectId,processDefineId, searchVal, statesStr, host, start, end, pageInfo.getStart(), pageSize);
    ...
    public Result queryProcessInstanceNodeResult(User loginUser, String projectName, Integer processInstanceId,
                                                        String name, int limit) {
        Result result = new Result();
        if(limit == 0) { limit = 100; }
        TaskInstance taskInstance = taskInstanceMapper.queryByInstanceIdAndName2(processInstanceId,  name);
     ...
     * @return
     */
    public Map<String, Object> queryTaskListByProcessId(User loginUser, String projectName, Integer processId)  {
        ...
        //获取 依赖类型 和 运行的节点
        ProcessInstance processInstance = processDao.findProcessInstanceDetailById(processId);
        String taskDependType = processInstance.getTaskDependType().toString();
        JSONObject cmdParamObj = (JSONObject) JSONObject.parse(processInstance.getCommandParam());
        String nodeName = (String)cmdParamObj.get("StartNodeNameList");
        JSONArray connects= JSONArray.parseArray(processInstance.getConnects());
        JSONObject locations = (JSONObject) JSONObject.parse(processInstance.getLocations());
        JSONObject processInstanceJson = (JSONObject) JSONObject.parse(processInstance.getProcessInstanceJson());
        String taskId="";
        //寻找 执行节点 taskId
        JSONArray tasks = processInstanceJson.getJSONArray("tasks");
        for(int i=0;i<tasks.size();i++){
            JSONObject jb = (JSONObject) JSONObject.parse(tasks.get(i).toString());
            if(jb.getString("name").equals(nodeName)){
                taskId=jb.getString("id");
            }
        }
        ...
        // 向前  TASK_ONLY, TASK_PRE, TASK_POST
        if(taskDependType.equals("TASK_PRE")){
            List<String> getTaskId = getTaskId(connects, taskId, list,"TASK_PRE");
            String taskName = getTaskName(tasks, getTaskId);
            taskInstanceList = processDao.findValidTaskListByProcessId2(processId,taskName);
            List<TaskInstance> validTaskListByProcessIdIn = processDao.findValidTaskListByProcessIdIn(processId, taskName);
            taskInstanceList.addAll(validTaskListByProcessIdIn);

        }//向后
        else  if(taskDependType.equals("TASK_POST")&&cmdParamObj.size()!=0){
            List<String> getTaskId = getTaskId(connects, taskId, list,"TASK_POST");
            String taskName = getTaskName(tasks, getTaskId);
            taskInstanceList = processDao.findValidTaskListByProcessId2(processId,taskName);
            List<TaskInstance> validTaskListByProcessIdIn = processDao.findValidTaskListByProcessIdIn(processId, taskName);
            taskInstanceList.addAll(validTaskListByProcessIdIn);
        }//当前
        else if(taskDependType.equals("TASK_ONLY")){
            ...
            taskInstanceList = processDao.findValidTaskListByProcessId(processId);
        }
...
//        List<TaskInstance> taskInstanceList = processDao.findValidTaskListByProcessId(processId);
    ...
    /**
     * query task list by process Definition Id
     *
     * @param loginUser
     * @param projectName
     * @param processId
     * @return
     */
    public Map<String, Object> queryTaskListByProcessDefinitionId(User loginUser, String projectName, Integer processId){
        Map<String, Object> result = new HashMap<>();
        Project project = projectMapper.queryByNameAndUser(projectName,loginUser.getId());
        ...
        int maxInstanceId;
        String maxInstance = processDefineMapper.getMaxInstanceId(processId);
        if(null==maxInstance || maxInstance.equals("")){
            putMsg(result, Status.WORK_ISNULL);
            return result;
        }else{
            maxInstanceId=Integer.parseInt(maxInstance);

        }

        ProcessInstance processInstance = processDao.findProcessInstanceDetailById(maxInstanceId);
        List<TaskInstance> taskInstanceList = processDao.findValidTaskListByProcessId2(maxInstanceId,"'X不存在X'");

        for(TaskInstance taskInstance :taskInstanceList){
            String taskType = taskInstance.getTaskType();
            if(taskType.equals("SPARK")&&taskInstance.getState().typeIsRunning()){

                RestTemplate restTemplate = new RestTemplate();
                String resourceManager = ConfigurationManager.getProperty(Constants.RESOURCEMANAGER);
                String url = resourceManager+taskInstance.getAppLink();
                logger.info("get resourceManager url : "+url);
                try{
                    ResponseEntity<String> exchange = restTemplate.exchange(url, HttpMethod.GET, null, String.class);
                    JSONObject js =JSONObject.parseObject(exchange.getBody());
        ...
        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put("instanceId", maxInstanceId);
        resultMap.put(PROCESS_INSTANCE_STATE, processInstance.getState().toString());
        resultMap.put(TASK_LIST, taskInstanceList);
        result.put(Constants.DATA_LIST, resultMap);
    ...
    public  static String getTaskName(JSONArray tasks,List<String> getTaskId){
        List list=new ArrayList();
        for(int i=0;i<tasks.size();i++){
            JSONObject jb = (JSONObject) JSONObject.parse(tasks.get(i).toString());
            for (String task:getTaskId) {
                if(jb.getString("id").equals(task)){
    ...
    //递归
    public static List getTaskId(JSONArray connects,String taskId,List list,String type){
        String endPointSourceId="";
        if(type.equals("TASK_POST")){//向后
            for(int i=0;i<connects.size();i++){
                String endPointTargetId = (String) connects.getJSONObject(i).get("endPointSourceId");
                if(taskId.equals(endPointTargetId)){

                    endPointSourceId=(String) connects.getJSONObject(i).get("endPointTargetId");
                    if(in==100){
                        break;
                    }
                    //添加 执行节点 上游 taskID
                    list.add(endPointSourceId);
                    in++;
                    getTaskId(connects,endPointSourceId,list,"TASK_POST");
                }
            }
        }else {//向前
            for (int i = 0; i < connects.size(); i++) {
                String endPointTargetId = (String) connects.getJSONObject(i).get("endPointTargetId");
                if (taskId.equals(endPointTargetId)) {

                    endPointSourceId = (String) connects.getJSONObject(i).get("endPointSourceId");
                    if (in == 100) {
                        break;
                    }
                    //添加 执行节点 上游 taskID
                    list.add(endPointSourceId);
                    in++;
                    getTaskId(connects, endPointSourceId, list,"TASK_PRE");
                }
            }
        }
