...
public class ResourcesService extends BaseService {
     ...
     */
    @Transactional(value = "TransactionManager",rollbackFor = Exception.class)
    public Result createFtpResource(MultipartFile file) {
        Result result = new Result();
...
//        }
        ...
        String path = ConfigurationManager.getProperty(Constants.EXCLEPATH);
        // 输出的文件流保存到本地文件
        ...
        for(String s:list){
            if(fileName.equals(s)){
                ...
                putMsg(result, Status.FILE_NAME_EXISTS);
                return result;
            }
        }
...
//          "D:\\test\\";
            ...
            result.setData(ConfigurationManager.getProperty(Constants.EXCLEPATH)+fileName);
            putMsg(result, Status.SUCCESS);
            return result;
            ...
            e.printStackTrace();
            putMsg(result, Status.EXCLE_CODE_ERROR);
            return result;
        }finally {
            // 完毕，关闭所有链接
            try {
    ...
    public Map<String, Object> queryResourceListPaging(User loginUser, ResourceType type, String searchVal, Integer pageNo, Integer pageSize) {
        ...
        if (isAdmin(loginUser)) {
            count = resourcesMapper.countAllResourceNumberByType(type.ordinal(),searchVal);
            resourceList = resourcesMapper.queryAllResourceListPaging(type.ordinal(), searchVal,
                    pageInfo.getStart(), pageSize);
        } else {
            count = resourcesMapper.countResourceNumberByType(loginUser.getId(), type.ordinal(),searchVal);
